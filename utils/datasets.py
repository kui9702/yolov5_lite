...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        # self.img_size = img_size  # 修改为 (640 352)   kile
        if kile_debug:
            self.img_size = (640, 352)
        else:
            self.img_size = img_size
        ...
        # self.mosaic_border = [-img_size // 2, -img_size // 2] # 修改如下   kile
        if kile_debug:
...
def letterbox(img, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True, stride=32):
    ...
    # 这里对齐 shape为[高 宽]  new_shape为[宽 高]   kile
    if kile_debug:
    ...
    new_unpad = int(round(shape[1] * r)), int(round(shape[0] * r))    # shape[0] 高  shape[1] 宽  new_unpad[0] 高  new_unpad[1] 宽
